CLASS zAJbb_cl_fill_table_data DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  PUBLIC SECTION.
...
CLASS zajbb_cl_fill_table_data IMPLEMENTATION.

  METHOD fill_bookings.

    GET TIME STAMP FIELD DATA(lv_createdat).
    TYPES t_booking_list       TYPE TABLE OF zAJ_dt_bookings   WITH EMPTY KEY.
    DELETE FROM zAJ_dt_bookings.
    ...
    INSERT zAJ_dt_bookings FROM TABLE @booking_list.
  ...
  METHOD fill_location.
    GET TIME STAMP FIELD DATA(lv_createdat).
    TYPES t_location_list       TYPE TABLE OF zAJ_dt_location   WITH EMPTY KEY.
    DELETE FROM zAJ_dt_location.
...
*      )
      (
        ...
        description  = |The Rijksmuseum is one of Amsterdam’s grandest and most popular museums. | &&
                       |Its vast collection showcases iconic art and a wide variety of artefacts that reflect more than 8AJ years of Dutch and global history, | &&
...
*        logo_url     = |https://www.bankgiroloterij.nl/upload_mm/youtubeelement3/d/8/previewImage727672.JPG|
    ...
    INSERT zAJ_dt_location FROM TABLE @location_list.
  ENDMETHOD.
